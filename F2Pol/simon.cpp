...
int main() {
     ...
     * Либо генерируются для Simon с помощью get_equations,
     * либо считываются из файла
     */

    // Считывание системы уравнений из файла
    // Описание parse_equations_from_file можно посмотреть в equations.h
    vector<Polynomial> equations = parse_equations_from_file("simon_equations.txt", false);

    // Выводим все уравнения
    print_equations(equations, true);

    /* Чтобы использовать атаку XL, нужно подключить файл XL_attack.h
     * Описание XL атаки лежит в файле XL_attack.h
     * Описание линеаризации в файле equations.h
    */

    XL_attack(equations);

    /* Чтобы использовать атаку Buchberger, нужно подключить файл buchberger.h
     * Описание атаки лежит в файле buchberger.h
     * Реализован самый простой вариант
    */

    // equations = Buchberger(equations);
    // print_equations(equations, true);
}