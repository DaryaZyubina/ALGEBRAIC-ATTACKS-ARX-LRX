...
namespace F2Pol {
    ...
    bool operator<(const Term& t1, const Term& t2) {
        return t1.var_ids.size() == t2.var_ids.size() ? t1.id < t2.id : t1.var_ids.size() < t2.var_ids.size();
    }
...
int main()
{
    ...
    do
    {
        ...
        try
        {
            Equation_system M(matrix, terms.size(), matrix.size());
            M.gauss_elim();
            matrix = M.get_system();
        }
        ...
        int t = 0; // id in temp_values
        int id_now; // variable for value
        

        //если линейно, последний неозначенный моном
        //(а он - одна переменная) означить полиномом
        //без этого монома
        //поместить уравнение-означивание в D
        for (int i = 0; i < I; i++)
        {
            id_now = F2Pol::all_terms[equations[equations.size() - 1 - i].terms[0]->id]->var_ids[0];
            F2Pol::Polynomial pol_var({ F2Pol::add_term({id_now}) });
            F2Pol::all_vars[id_now]->value = &(add_pol(equations[equations.size() - 1 - i], pol_var));
            F2Pol::temp_values[t] = *F2Pol::all_vars[id_now]->value;
            t++;
            equations.pop_back();
            //выполнить подстановку переменной
            ...
            //возможно, в D появится численное значение
            //какой-то переменной, подставить
            //a.terms.size() == 1 && a.terms[0] == one_term
            /*for (auto eq : D)
            {
                if (eq.terms.size() == 1)
                {
                    all_vars[eq.terms[0]->var_ids[0]]->int_value = 0;
                }
                else if (eq.terms.size() == 2 && find(eq.terms.begin(), eq.terms.end(), one_term) != eq.terms.end())
                {

                }
            }*/
            D.push_back(*F2Pol::all_vars[id_now]->value);
        }
        /*продолжать подниматься, пока так не рассмотрим
    ...
    for (auto eq : D)
    {
        if (eq.terms.size() == 1)
        {
            F2Pol::all_vars[eq.terms[0]->var_ids[0]]->int_value = 0;
        }
        else if (eq.terms.size() == 2 && find(eq.terms.begin(), eq.terms.end(), F2Pol::one_term) != eq.terms.end())
        {
            F2Pol::all_vars[eq.terms[0]->var_ids[0]]->int_value = 1;
            F2Pol::all_vars[eq.terms[1]->var_ids[0]]->int_value = 1;
        }
    }
