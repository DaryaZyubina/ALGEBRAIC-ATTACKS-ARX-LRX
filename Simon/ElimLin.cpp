...
int main()
{
    ...
    auto equations = F2Pol::parse_equations_from_file("speck.txt");
    int cnt_linear = 0;
    for (auto it : equations) {
        if (F2Pol::is_linear_pol(it)) {
            cnt_linear++;
        }
    }
    cout << equations.size() << " " << cnt_linear << endl;
...
*/
    ...
    do
    {
        ...
        cout << "!!!!!!!! " << matrix.size() << " " << equations.size() << " " << D.size() << endl;
        if (equations.size() == 1) {
            cout << equations[0] << endl;
        }
        /*cout << "terms:" << "\n";
        for (auto it : terms) {
            cout << *it << " ";
        ...
        cout << "??????? " << matrix.size() << " " << equations.size() << endl;
        for (auto it : equations) {
            cout << it << "\n";
        }
        cout << "\n";
        ...
        for (int j = (int)equations.size() - 1; j >= 0; j--)
        {
        ...
        for (int i = 0; i < I; i++)
        {
            ...
            id_now = equations.back().terms[0]->var_ids[0];
            cout << "! " << id_now << " " << *F2Pol::all_vars[id_now] << endl;

            // signify variable by polinom
            F2Pol::Polynomial pol_var({ F2Pol::add_term({id_now}) });
            D.push_back(make_pair(id_now, add_pol(equations.back(), pol_var)));
            ...
            F2Pol::eval_id += 1;
            F2Pol::temp_values[id_now] = F2Pol::Polynomial(D.back().second);
    ...
    Equation_system M(matrix, num_columns, matrix.size());
    M.find_solution();
    vector<vector<int> > solutions = M.get_solutions();
